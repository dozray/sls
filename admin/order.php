...
//获取登陆管理员是供应商 还是总管理
 $huoquguanli = $db->GetOne('SELECT role_id FROM ' . $ecs->table('admin_user') . " WHERE user_id = '" .$admin_id. "'");
    ...
    $sql = "SELECT o.*, IF(o.product_id > 0, p.product_number, g.goods_number) AS storage, o.goods_attr, g.suppliers_id, g.admin_id, IFNULL(b.brand_name, '') AS brand_name, p.product_sn
            ...
            WHERE o.order_id = '$order[order_id]' and g.admin_id = '$admin_id'";
	}else{
	 $sql = "SELECT o.*, IF(o.product_id > 0, p.product_number, g.goods_number) AS storage, o.goods_attr, g.suppliers_id, g.admin_id, IFNULL(b.brand_name, '') AS brand_name, p.product_sn
            FROM " . $ecs->table('order_goods') . " AS o
                LEFT JOIN " . $ecs->table('products') . " AS p
    ...
    $res = $db->query($sql);
	 /*function gonghuoshangname($adminid){
 
		$gongyingshang ="select name from ecs_admin_user where role_id=1 and user_id=$adminid";
		
		$res = $db->query($gongyingshang);
		echo $res;exit;
		$fanhui = $db->fetchRow($res);
		return $fanhui['name'];
 }*/
    while ($row = $db->fetchRow($res))
    {
		...
		/*echo $row['admin_id'];
		$row['gonghuoshang'] = gonghuoshangname($row['admin_id']);*/

		$gongyingshang ="select name from ecs_admin_user where role_id=1 and user_id='".$row['admin_id']."'";
		$row1 = $GLOBALS['db']->getRow($gongyingshang);
		$row['gonghuoname'] = $row1['name'];
...
elseif ($_REQUEST['act'] == 'operate')
{
	...
	//统计亭子
	 elseif (isset($_POST['tongjitingzi']))
    {
       ...
       $sql="SELECT oi.tingziId, og.goods_name, SUM( og.goods_number ) AS gsum FROM ecs_order_info AS oi LEFT JOIN ecs_order_goods AS og ON oi.order_id = og.order_id WHERE oi.order_sn IN ( ".$_POST['order_id']." )  GROUP BY oi.tingziId, og.goods_name ";
        ...
        $smarty->assign('goods_list',$goods_list);
		 $smarty->template_dir = '../' . DATA_DIR;
            $html .= $smarty->fetch('order_tongjitingzi.html') .
                '<div style="PAGE-BREAK-AFTER:always"></div>';
				echo $html;
	        exit;
    }
	//统计货物
	 elseif (isset($_POST['tongjihuowu']))
    {
       ...
       $sql="SELECT og.goods_name, oi.tingziId, SUM( og.goods_number ) AS gsum FROM ecs_order_info AS oi LEFT JOIN ecs_order_goods AS og ON oi.order_id = og.order_id WHERE oi.order_sn IN ( ".$_POST['order_id']." )  GROUP BY oi.tingziId, og.goods_name ORDER BY og.goods_name ";
		$res = $db->query($sql);
		 while ($row = $db->fetchRow($res))
            {
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        //区域管理员只看到自己的订单（如果用户下单商品包括多个区域管理下的商品也可以看到）
        if( $_SESSION['province'] ){
            $where .= " and o.order_id in (select order_id from " . $GLOBALS['ecs']->table('order_goods') . " as og," . $GLOBALS['ecs']->table('goods') . " as g where g.admin_id = ".$_SESSION['admin_id'] ." and og.goods_id=g.goods_id)";
