...
if ($_REQUEST['act'] == 'list')
{
    ...
    if($province){
        $sql .= " and order_id in (select order_id from ".$ecs->table('order_goods')." as og,".$ecs->table('goods')." as g where og.goods_id=g.goods_id and g.admin_id=".$_SESSION['admin_id'].") ";
    }
    ...
    if ($is_multi)
    {
        ...
        foreach($start_date_arr AS $k => $val)
        {
           ...
           $sql.= "WHERE p.pay_id = i.pay_id AND i.order_status = '" .OS_CONFIRMED. "' ".
         ...
         if($province){
        $sql .= " and i.order_id in (select order_id from ".$ecs->table('order_goods')." as og,".$ecs->table('goods')." as g where og.goods_id=g.goods_id and g.admin_id=".$_SESSION['admin_id'].") ";
    }
             $sql.= " GROUP BY i.pay_id ORDER BY order_num DESC";
        ...
        foreach($start_date_arr AS $k => $val)
        {
             ...
             $sql .= ' WHERE sp.shipping_id = i.shipping_id ' . order_query_sql('finished') .
               "AND i.add_time >= '$start_date_arr[$k]' AND i.add_time <= '$end_date_arr[$k]' " ;
         if($province){
        $sql .= " and i.order_id in (select order_id from ".$ecs->table('order_goods')." as og,".$ecs->table('goods')." as g where og.goods_id=g.goods_id and g.admin_id=".$_SESSION['admin_id'].") ";
    }
             $sql .=" GROUP BY i.shipping_id ORDER BY order_num DESC";
    ...
    else
    {
        ...
        $sql .= " WHERE p.pay_id = i.pay_id " . order_query_sql('finished') .
...
/*------------------------------------------------------ */
 ...
 function get_orderinfo($start_date, $end_date)
 {
    ...
    /* 未确认订单数 */
     ...
     if($province){
        $sql .= " and order_id in (select order_id from ".$GLOBALS['ecs']->table('order_goods')." as og,".$GLOBALS['ecs']->table('goods')." as g where og.goods_id=g.goods_id and g.admin_id=".$_SESSION['admin_id'].")";
    }
