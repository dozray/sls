...
if ($_REQUEST['act'] == 'list')
{
    ...
    $city = intval($_SESSION['city']);
    $district = intval($_SESSION['district']);
    if( $province > 0 ) {
        $sql = " SELECT IFNULL(SUM(a.user_money), 0) AS user_money, IFNULL(SUM(a.frozen_money), 0) AS frozen_money FROM " .
                $ecs->table('account_log') . " as a, ".$ecs->table('users')." as u WHERE a.user_id = u.user_id and u.province=".$province." and u.city=".$city." and u.district=".$district." and a.`change_time` >= " . $start_date ." AND a.`change_time` < " .($end_date+86400);
    } else {
    ...
    if( $province > 0 ) {
        $sql = "SELECT IFNULL(SUM(o.surplus), 0) AS surplus, IFNULL(SUM(o.integral_money), 0) AS integral_money FROM ".
                $ecs->table('order_info') ." as o,".$ecs->table('users')." as u WHERE o.user_id = u.user_id and u.province=".$province." and u.city=".$city." and u.district=".$district." AND o.`add_time` >= " . $start_date ." AND o.`add_time` < " .($end_date+86400);
    } else{
...
function get_total_amount ($start_date, $end_date, $type=0)
{
    ...
    $district = intval($_SESSION['district']);
    if( $province > 0 ){
        $sql .= " and u.province=".$province." and u.city=".$city." and u.district=".$district;
    }
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        $district = intval($_SESSION['district']);
        if( $province > 0 ){
            $ex_where .= " and u.province=".$province." and u.city=".$city." and u.district=".$district;
        }
