...
function set_admin_session($row)
{
    ...
    $_SESSION['last_check']  = $row['last_time']; // 用于保存最后一次检查订单的时间
...
function admin_shop_list(){
    $sql = "select name, user_name,user_id,role_id from ".$GLOBALS['ecs']->table('admin_user')." where role_id = 1";
