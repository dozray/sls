...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        $filter['admin_shop_id'] = isset($_REQUEST['admin_shop_id']) ? (empty($_REQUEST['admin_shop_id']) ? '' : trim($_REQUEST['admin_shop_id'])) : '';
        ...
        /* 供货商 */
        if (!empty($filter['admin_shop_id']))
        {
            $where .= " AND (admin_id = '" . $filter['admin_shop_id'] . "')";
        }
