...
if ($_REQUEST['act'] == 'list' || $_REQUEST['act'] == 'trash')
{
    ...
    $admin_shop_id = isset($_REQUEST['admin_shop_id']) ? (empty($_REQUEST['admin_shop_id']) ? '' : trim($_REQUEST['admin_shop_id'])) : '';
    ...
    $admin_shops = admin_shop_list();
    $smarty->assign('admin_shops', $admin_shops);
    $smarty->assign('admin_shop_id', $admin_shop_id);
 ...
 */
    if($_SESSION['role_id'] == 1){
        $conditions = " and admin_id = ".$_SESSION['admin_id'];
    }elseif($admin_shop_id){
        $conditions = " and admin_id = ".$admin_shop_id;
    }
 
    $goods_list = goods_list($_REQUEST['act'] == 'list' ? 0 : 1, ($_REQUEST['act'] == 'list') ? (($code == '') ? 1 : 0) : -1, $conditions);
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    $admin_shops = $_SESSION['role_id'] == '1' ? false : admin_shop_list();
    $smarty->assign('admin_shops', $admin_shops);
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
	...
	$dai_percent = isset($_POST['dai_percent']) ? intval($_POST['dai_percent']) : 0;
	$shou_percent = isset($_POST['shou_percent']) ? intval($_POST['shou_percent']) : 0;
	
	$max_number = isset($_POST['max_number']) ? intval($_POST['max_number']) : 0;
	
	$chandi = isset($_POST['chandi']) ? $_POST['chandi'] : '';
	$guige = isset($_POST['guige']) ? $_POST['guige'] : '';
	
	$admin_id = $_SESSION['role_id'] == '1' ? intval($_SESSION['admin_id']) : intval($_POST['admin_id']);
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "suppliers_id, dai_percent, shou_percent, admin_id, max_number, chandi, guige)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    " '$rank_integral', '$suppliers_id', '$dai_percent', '$shou_percent', '$admin_id', ".
                    " '$max_number', '$chandi', '$guige')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, " .
                    "extension_code, rank_integral, dai_percent, shou_percent, admin_id, max_number, chandi, guige)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    ...
                    " '$rank_integral', '$dai_percent', '$shou_percent', '$admin_id', '$max_number', '$chandi', '$guige')";
        }
    }
    else
    {
        ...
        $sql = "UPDATE " . $ecs->table('goods') . " SET " .
				...
				"dai_percent = '$dai_percent', " .
				"shou_percent = '$shou_percent', " .
