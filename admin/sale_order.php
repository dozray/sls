...
{ 
    $filter['start_date'] = empty($_REQUEST['start_date']) ? '' : strstr($_REQUEST['start_date'], '-') === false ? $_REQUEST['start_date'] : local_strtotime($_REQUEST['start_date']);
    $filter['end_date'] = empty($_REQUEST['end_date']) ? '' : strstr($_REQUEST['_REQUEST'], '-') === false ? $_REQUEST['_REQUEST'] : local_strtotime($_REQUEST['end_date']);
    $filter['sort_by'] = empty($_REQUEST['sort_by']) ? 'goods_num' : trim($_REQUEST['sort_by']);
    $filter['sort_order'] = empty($_REQUEST['sort_order']) ? 'DESC' : trim($_REQUEST['sort_order']);
    
    $where = " WHERE 1 ". order_query_sql('finished', 'oi.');
    ...
    if($province){
        $where .=" and og.goods_id in(select goods_id from ". $GLOBALS['ecs']->table('goods') ." where admin_id=".$_SESSION['admin_id'].") ";
    }
    ...
    $sql = "SELECT distinct(og.goods_id), og.goods_sn, og.goods_name, oi.order_status, " .
