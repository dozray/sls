...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$ecs->table('admin_user')." (user_name, name, email, password, add_time, nav_list, action_list, role_id, country, province, city, district, uname, phone, mobile, address, info) ".
           "VALUES ('".trim($_POST['user_name'])."', '".trim($_POST['name'])."', '".trim($_POST['email'])."', '$password', '$add_time', '$row[nav_list]', '$action_list', '$role_id', '$country', '$province', '$city', '$district', '$uname', '$phone', '$mobile', '$address', '$info')";
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $user_info['country'] = $user_info['country'] || 1;
    ...
    $smarty->assign('country_list',       get_regions());
    $smarty->assign('province_list', get_regions(1, intval($user_info['country'])));
    $smarty->assign('city_list',     get_regions(2, intval($user_info['province'])));
    $smarty->assign('district_list', get_regions(3, intval($user_info['city'])));
...
elseif ($_REQUEST['act'] == 'info'){
	...
	$user_info['areaname'] = '';
        if($user_info['province']){
            $user_info['areaname'] .= $db->getOne('select region_name from '.$ecs->table('region').' where region_id='.$user_info['province']);
        }
        if($user_info['city']){
            $user_info['areaname'] .= $db->getOne('select region_name from '.$ecs->table('region').' where region_id='.$user_info['city']);
        }
        if($user_info['district']){
            $user_info['areaname'] .= $db->getOne('select region_name from '.$ecs->table('region').' where region_id='.$user_info['district']);
        }
    ...
    assign_query_info();
    $smarty->display('privilege_info2.htm');
...
elseif ($_REQUEST['act'] == 'modif')
{
    ...
    /* 获得当前管理员数据信息 */
    $sql = "SELECT user_id, user_name, name, email, nav_list ".
...
{
    $list = array();
    $sql  = 'SELECT user_id, name, user_name, email, add_time, last_login, province,city,district, role_id '.
    ...
    foreach ($list AS $key=>$val)
    {
        $list[$key]['areaname'] = '';
        if($val['province']){
            $list[$key]['areaname'] .= $GLOBALS['db']->getOne('select region_name from '.$GLOBALS['ecs']->table('region').' where region_id='.$val['province']);
        }
        if($val['city']){
            $list[$key]['areaname'] .= $GLOBALS['db']->getOne('select region_name from '.$GLOBALS['ecs']->table('region').' where region_id='.$val['city']);
        }
        if($val['district']){
            $list[$key]['areaname'] .= $GLOBALS['db']->getOne('select region_name from '.$GLOBALS['ecs']->table('region').' where region_id='.$val['district']);
        }
