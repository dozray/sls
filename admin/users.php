...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    $city = 0;
    $district = 0;
    $smarty->assign('country_id', $country);
    $smarty->assign('province_id', $province);
    $smarty->assign('city_id', $city);
    $smarty->assign('district_id', $district);
    $smarty->assign('country_list',       get_regions());
    $smarty->assign('province_list', get_regions(1, $country));
...
elseif ($_REQUEST['act'] == 'insert')
{
   ...
   if( $country > 0 && $province > 0 ) {
    ...
    $other['user_rank']  = $rank==3?3:0;
    $other['user_rankz'] = $rank==2?1:0;
    $other['sex']        = $sex;
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT u.user_name, u.sex, u.birthday, u.pay_points, u.rank_points, u.user_rank , u.user_rankz, u.user_money, u.frozen_money, u.credit_line, u.parent_id, u2.user_name as parent_username, u.qq, u.msn, u.office_phone, u.home_phone, u.mobile_phone".
    ...
    if ($row)
    {
        ...
        $user['user_rankz'] = $row['user_rankz'];
...
//        $user['formated_frozen_money'] = price_format(0);
    ...
    $country = $province = $city = $district = 0;
    $sql = 'SELECT * FROM ' . $ecs->table('user_address') . ' where user_id = '.$user['user_id'].' order by address_id desc limit 1';
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    if( $country > 0 && $province > 0 ) {
        $sql = "update ". $ecs->table('user_address') ." set province = '{$province}',city='{$city}',district='{$district}' where user_id='$user_id' ";
        $db->query($sql);
    }
...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        {
            if($filter['rank'] == 1){
                   $ex_where .= " and user_rank = 0 and user_rankz = 0 ";
            }elseif($filter['rank'] == 2){
                $ex_where .= " and user_rank = 0 and user_rankz = 1 ";
            }elseif($filter['rank'] == 3){
                $ex_where .= " and user_rank = 3 ";
            }
        ...
        if($_SESSION['province']){
            $ex_where .=" and province='$_SESSION[province]' ";
        }
        if($_SESSION['city']){
            $ex_where .=" and city='$_SESSION[city]' ";
        }
        if($_SESSION['district']){
            $ex_where .=" and district='$_SESSION[district]' ";
        }
        ...
        $sql = "SELECT user_id, user_name, email, is_validated, user_rank, user_rankz, user_money, frozen_money, rank_points, pay_points, reg_time ".
    ...
    for ($i=0; $i<$count; $i++)
    {
        $user_list[$i]['areaname'] = '';
        $address = $GLOBALS['db']->getRow('select province,city,district,name from '.$GLOBALS['ecs']->table('user_address').' where user_id='.$user_list[$i]['user_id']);
        if($address){
            if($address['province']){
                $user_list[$i]['areaname'] .= $GLOBALS['db']->getOne('select region_name from '.$GLOBALS['ecs']->table('region').' where region_id='.$address['province']);
            }
            if($address['city']){
                $user_list[$i]['areaname'] .= $GLOBALS['db']->getOne('select region_name from '.$GLOBALS['ecs']->table('region').' where region_id='.$address['city']);
            }
            if($address['district']){
                $user_list[$i]['areaname'] .= $GLOBALS['db']->getOne('select region_name from '.$GLOBALS['ecs']->table('region').' where region_id='.$address['district']);
            }
            $user_list[$i]['wangdian'] = $address['name'];
        }
