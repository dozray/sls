...
//{
    ...
    assign_cat_goods(26,3); //应季水果
    assign_cat_goods(25,3); //海鲜水产
    assign_cat_goods(23,3); //粮油

    include_once(ROOT_PATH .'includes/lib_clips.php');
    include_once(ROOT_PATH .'includes/lib_transaction.php');
    $uinfo = get_user_default($_SESSION['user_id']);
	
    $smarty->assign('uinfo', ($uinfo && $uinfo['user_id']>0) ? $uinfo : false);
	
	//获取一个订单
	if($_SESSION['user_id']){
		$sql = "select order_id,order_sn,order_status,shipping_status, pay_status from ".$ecs->table("order_info")." where user_id = ". intval($_SESSION['user_id']);
		$orders = $db->getRow($sql);
		$orders_goodsname = '';
		if($orders){
			$sql = "select goods_name from ".$ecs->table('order_goods')." where order_id = ".$orders['order_id'];
			$order_goods = $db->getAll($sql);
			foreach($order_goods as $goods){
				$order_goodsname .= $goods['goods_name'].' ';
			}
		...
		$smarty->assign('order_goodsname', $order_goodsname);
	}
