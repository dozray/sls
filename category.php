...
$sort  = (isset($_REQUEST['sort'])  && in_array(trim(strtolower($_REQUEST['sort'])), array('goods_id', 'goods_sale', 'shop_price', 'last_update'))) ? trim($_REQUEST['sort'])  : $default_sort_order_type;
...
if (!$smarty->is_cached('category.dwt'))
{
    ...
    if(empty($child_tree)){
        $parent_id = get_parentid($cat_id);
        $child_tree = get_child_tree($parent_id);
    }
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, g.goods_number, g.max_number, ' .
                "g.shop_price, g.promote_price, g.goods_type, " .
                'g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb , g.goods_img, g.chandi, g.guige ' .
               // ',count(og.goods_number) as goods_sale '.
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            /*'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " 
            'LEFT JOIN '. $GLOBALS['ecs']->table('order_goods') . ' AS og ' .
                "ON og.goods_id = g.goods_id "..*/
            //"WHERE $where $ext GROUP BY og.goods_id ORDER BY $sort $order";
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[$row['goods_id']]['max_number'] = $row['max_number'];
        $arr[$row['goods_id']]['chandi'] = $row['chandi'];
