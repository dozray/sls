...
if (empty($_SESSION['user_id']))
{
	...
	$rank = get_rank_info();
	if(!isset($rank['next_rank'])) {
		$special_rank = 1;
	}
}
 $smarty->assign('special_rank',       $special_rank);
/* 如果是显示页面，对页面进行相应赋值 */
if (in_array($action, $ui_arr))
{
    assign_template();
...
if ($action == 'register')
{
    ...
    $smarty->assign('get_shops', $get_shops);
	//print_r($get_shops);exit;
    /* 密码提示问题 */
...
elseif ($action == 'dgorder_list')
{
	...
	$quhuo_name = trim($_GET['quhuo_name']);
	$quhuo_tel = trim($_GET['quhuo_tel']);
	$where = "";
	
	if($order_number != ''){
		$where .= " and order_sn = '{$order_number}' ";
	}
	if($order_status != '-1' && $order_status != ''){
		$where .= " and order_status = '{$order_status}' ";
	}
	if($user_name != ''){
		$where .= " and user_id =".$db->getOne("select user_id from ".$ecs->table('users')." where user_name = '".$user_name."'")." ";
	}
	if($quhuo_name != ''){
		$where .= " and quhuo_name = '{$quhuo_name}' ";
	}
	if($quhuo_tel != ''){
		$where .= " and quhuo_tel = '{$quhuo_tel}' ";
	}
    $record_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('order_info'). " WHERE daigou_id = '$user_id' ".$where);

    $pager  = get_pager('user.php', array('act' => $action), $record_count, $page);

    $orders = get_user_orders($user_id, $pager['size'], $pager['start'], 1, $where);
	
	if( $orders ) {
		foreach($orders as $k => $order) {
			$order['gou_money'] = 0;
			$goods = $db->getAll("select goods_id,goods_name,goods_number,goods_price from ".$ecs->table('order_goods')." where order_id = ".$order['order_id']);
			if( $goods ) {
				foreach( $goods as $gk => $good) {
					$goodsinfo = $db->getOne("select dai_percent from ".$ecs->table('goods')." where goods_id = ".$good['goods_id']);
					if( $goodsinfo) {
						$good['dai_percent'] = $goodsinfo['dai_percent'];
					} else {
						$good['dai_percent'] = 0;
					}
					
					$order['gou_money'] += round($good['goods_price'] * $good['dai_percent'],2);
					$good['goods_price'] = price_format($good['goods_price'], false);
					$goods[$gk] = $good;
				}
			}
			//实际购买人
			$order['users'] = $db->getRow("select * from ".$ecs->table('users')." where user_id = ".$order['user_id']);
			
			$order['gou_money'] = price_format($order['gou_money'], false);
			$order['goods'] = $goods;
			$orders[$k] = $order;
		}
	...
	$smarty->assign('quhuo_name', $quhuo_name);
	$smarty->assign('quhuo_tel', $quhuo_tel);
    $smarty->display('user_transaction.dwt');
}
//代收订单
elseif ($action == 'dsorder_list')
{
    include_once(ROOT_PATH . 'includes/lib_transaction.php');
    ...
    $record_count = $db->getOne("SELECT COUNT(*) FROM " .$ecs->table('order_info'). " WHERE daishou_id = '$user_id' ".$where);

    $pager  = get_pager('user.php', array('act' => $action), $record_count, $page);

    $orders = get_user_orders($user_id, $pager['size'], $pager['start'], 2, $where);
	...
	if( $orders ) {
		foreach($orders as $k => $order) {
			...
			if( $goods ) {
				foreach( $goods as $gk => $good) {
					$goodsinfo = $db->getOne("select shou_percent from ".$ecs->table('goods')." where goods_id = ".$good['goods_id']);
					if( $goodsinfo) {
						$good['shou_percent'] = $goodsinfo['shou_percent'];
					} else {
						$good['shou_percent'] = 0;
					}
					$order['shou_money'] += round($good['goods_price'] * $good['shou_percent'],2);
					$good['goods_price'] = price_format($good['goods_price'], false);
					$goods[$gk] = $good;
				}
			}
			//实际购买人
			$order['users'] = $db->getRow("select user_name from ".$ecs->table('users')." where user_id = ".$order['user_id']);
			
			$order['shou_money'] = price_format($order['shou_money'], false);
			$order['goods'] = $goods;
			$orders[$k] = $order;
		}
...
elseif ($action == 'address_list')
{
    ...
    $smarty->assign('lang',  $_LANG);
	$rank = get_rank_info();
	$specail_rank = !isset($rank['next_rank']) ? 1 : 0;

	$smarty->assign('specail_rank', $specail_rank);
...
/* 网点更改订单为到货*/
elseif ($action == 'affirm_daohuo')
{
    include_once(ROOT_PATH . 'includes/lib_transaction.php');

    $order_id = isset($_GET['order_id']) ? intval($_GET['order_id']) : 0;
    $from = isset($_GET['from']) ? $_GET['from'] : 'ds';

    if (affirm_daohuo($order_id, $user_id))
    {
        ecs_header("Location: user.php?act=".$from."order_list\n");
        exit;
    }
    else
    {
        $err->show($_LANG['order_list_lnk'], 'user.php?act='.$from.'order_list');
    }
}
/* 网点更改订单为客户收货*/
elseif ($action == 'affirm_shouhuo')
{    $quhuo_ma  = $_POST['quhuo_ma'];
    include_once(ROOT_PATH . 'includes/lib_transaction.php');

    $order_id = isset($_GET['order_id']) ? intval($_GET['order_id']) : 0;
    $from = isset($_GET['from']) ? $_GET['from'] : 'ds';
	 $_POST['123'] = isset($_GET['quhuo_ma']) ? htmlspecialchars($_GET['quhuo_ma']) : '';

    if (affirm_shouhuo($order_id, $user_id,$quhuo_ma))
    {
        ecs_header("Location: user.php?act=".$from."order_list\n");
        exit;
    }
