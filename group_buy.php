...
if ($_REQUEST['act'] == 'list')
{
    ...
    {
        $todaygoods_list = group_buy_list(4, 1);
        $smarty->assign('todaygoods_list',  $todaygoods_list);
        if ($count > 0)
        {
            /* 取得当前页的团购活动 */
            $gb_list = group_buy_list($size, $page, 4);
 ...
 * @return  array
 */
function group_buy_list($size, $page, $de_offset=0)
{
    /* 取得团购活动 */
    $gb_list = array();
    $now = gmtime();
    ...
    $res = $GLOBALS['db']->selectLimit($sql, $size, ($page - 1) * $size+$de_offset);
    while ($group_buy = $GLOBALS['db']->fetchRow($res))
    {
        ...
        if (!is_array($price_ladder) || empty($price_ladder))
        {
            ...
            $group_buy['group_price'] = price_format($group_buy['shop_price']);
        }
        else
        {
        ...
        $group_buy['goods_buyer'] = get_goods_buyer( $group_buy['goods_id'] );
...
//一个商品购买人数（订单数）
function get_goods_buyer( $goods_id ) {
    
    require 'includes/lib_order.php';
    $sql = "select count(*) as count from ". $GLOBALS['ecs']->table('order_goods') . " as og,". $GLOBALS['ecs']->table('order_info') . " as o where og.goods_id=".$goods_id." and o.order_id = og.order_id ".order_query_sql()." ";
