...
if($_GET['phone'])
{
    ...
    $yonghudianhua = $db->getOne($sql);
    $rand = rand(100000,999999);
    $_SESSION['phone_code'] = $rand;
	$sendid='5375';		
	$neirong=array($rand,'60');
	include_once('includes/Send.php');
	sendTemplateSMS($yonghudianhua,$neirong,$sendid);
...
if ($_REQUEST['step'] == 'add_to_cart')
{
	...
	if($cart_goods){
		foreach($cart_goods as $k => $goods){
			$goodsinfo = $db->getRow('select goods_thumb , goods_img from '.$ecs->table('goods').' where goods_id = '.$goods['goods_id']);
			$goods['goods_thumb']      = get_image_path($goodsinfo['goods_id'], $goodsinfo['goods_thumb'], true);
			$goods['goods_img']        = get_image_path($goodsinfo['goods_id'], $goodsinfo['goods_img']);
			$cart_goodsnum += $goods['goods_number'];
			$cart_goodsprice += $goods['subtotal'];
			$cart_goods[$k] = $goods;
		}
	}
	//$smarty->assign('cart_goodsnum', $cart_goodsnum);
	//$smarty->assign('cart_goodsprice', $cart_goodsprice);
...
elseif ($_REQUEST['step'] == 'consignee')
{
    ...
    else
    {
		...
		$daigou_id = intval($_POST['daigou_id']);
		$daishou_id = intval($_POST['daishou_id']);
		$shouhuo_type = intval($_POST['shouhuo_type']);
		if( $daigou_id < 1 && $daishou_id < 1) {
			$consignee = array(
			...
			//$consignee_list = get_consignee_list($daigou_id);
			//$consignee = $consignee_list[0];
		};
		//echo  $_POST['sendTime'];exit;

        /* 保存到session */
        $_SESSION['flow_consignee'] = stripslashes_deep($consignee);
		$_SESSION['daigou_id'] = $daigou_id;
		$_SESSION['daishou_id'] = $daishou_id;
		$_SESSION['shouhuo_type'] = $shouhuo_type;
		$_SESSION['gquhuo_name'] = $_POST['gquhuo_name'];
		$_SESSION['gquhuo_tel'] = $_POST['gquhuo_tel'];
		$_SESSION['gquhuo_fangshi'] = $_POST['gquhuo_fangshi'];
		$_SESSION['squhuo_name'] = $_POST['squhuo_name'];
		$_SESSION['squhuo_tel'] = $_POST['squhuo_tel'];
		$_SESSION['receiveaddres'] = $_POST['receiveaddres'];
		$_SESSION['sendTime'] = $_POST['sendTime'];
		$_SESSION['squhuo_fangshi'] = $_POST['squhuo_fangshi'];
...
elseif ( $_REQUEST['step'] == 'getuser') {
	...
	//$dg = substr($uns,$num+1);
	$daigou_id = $_SESSION['user_id'];
	//print_r($username);print_r($daigou_id);
	...
	if( $userInfo ) {
		...
		$sqll = "SELECT * FROM " . $GLOBALS['ecs']->table('users') .
         " WHERE province = '$province' and city = '$city' and district = '$district' and msn = '$msn' and user_id = '$daigou_id' LIMIT 5";
		...
		/*
			$consignee = $consignee_list[0];
			$result['countrys'] = get_regions();
			$consignee['country']  = isset($consignee['country'])  ? intval($consignee['country'])  : 0;
            $consignee['province'] = isset($consignee['province']) ? intval($consignee['province']) : 0;
            $consignee['city']     = isset($consignee['city'])     ? intval($consignee['city'])     : 0;

            $result['provinces'] = get_regions(1, $consignee['country']);
            $result['citys'] = get_regions(2, $consignee['province']);
            $result['districts'] = get_regions(3, $consignee['city']);
			$result['list'] = consignee;
			*/
		}
	} else {
		$result['info'] = '代购帐号不存在';
...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    $total = order_allfee($order, $cart_admin_goods, $consignee);
    ...
    foreach ($shipping_list AS $key => $val)
    {
        ...
        $ftotal = order_allfee($order, $cart_admin_goods, $consignee);
        
        $shipping_fee =  $ftotal['shipping_fee'];
...
elseif ($_REQUEST['step'] == 'select_shipping')
{
    ...
    else
    {
        ...
        /* 计算订单的费用 */
        $total = order_allfee($order, $cart_goods, $consignee);
        $smarty->assign('total', $total);
...
/*余额支付发送验证码的页面*/
elseif ($_REQUEST['step'] == 'checkout_pay')
{
    /*------------------------------------------------------ */
    //-- 订单确认
    /*------------------------------------------------------ */
	...
	*/
	...
	$mes = array('quhuo_name' => '', 'receiveaddres' => '', 'quhuo_tel' => '', 'quhuo_fangshi' => '', 'sendTime' => '', 'postscript' => '');
	$mes['quhuo_name'] = $_POST['quhuo_name'];
	$mes['receiveaddres'] = $_POST['receiveaddres'];
	$mes['quhuo_tel'] = $_POST['quhuo_tel'];
	$mes['quhuo_fangshi'] = $_POST['quhuo_fangshi'];
	$mes['sendTime'] = $_POST['sendTime'];
	$mes['postscript'] = $_POST['postscript'];
	...
	*/
	...
	$smarty->assign('step',            $_REQUEST['step']);
	assign_dynamic('shopping_flow');
	$smarty->display('flow_pay.dwt');exit();
    /* 保存 session
    $_SESSION['flow_order'] = $order; */
}
/*------------------------------------------------------ */
...
elseif ($_REQUEST['step'] == 'done')
{
    ...
    $order = array(
        ...
        'pay_status'      => PS_UNPAYED,
         'hebing_sn'      => get_order_sn(). rand(10000, 99999),
    ...
    /* 订单中的商品 */
    $cart_admin_goods = cart_admin_goods($flow_type);

    if (empty($cart_admin_goods))
    {
    ...
    $order_all_amount = 0;
    $total_amount = 0;
    $order_left_surplus = $order['surplus'];
    foreach($cart_admin_goods as $admin_id => $cart_goods){
        ...
        if ($payment['pay_code'] == 'balance' && $order['order_amount'] > 0)
        {
            ...
            {
                $order_left_surplus = $order['surplus'] - $order['order_amount'];
		...
		//exit;
		// 获取用户MSN为所属亭子
		$sql = "select msn from ".$ecs->table('users')." where user_id = ".$order['user_id'];
        $msn = $db->getOne($sql);
		//echo $msn;
		//exit;
        $order['daigou_id'] = isset($_SESSION['daigou_id']) ? intval($_SESSION['daigou_id']) : 0;
		 $order['tingziId'] = $msn;
        $order['daishou_id'] = isset($_SESSION['daishou_id']) ? intval($_SESSION['daishou_id']) : 0;
		if(isset($_SESSION['shouhuo_type'])){
			if($_SESSION['shouhuo_type'] == 1 || $_SESSION['shouhuo_type'] == 2){
				$order['quhuo_name'] = $_SESSION['shouhuo_type'] == 1 ? $_SESSION['gquhuo_name'] : $_SESSION['squhuo_name'];
				$order['quhuo_tel'] = $_SESSION['shouhuo_type'] == 1 ? $_SESSION['gquhuo_tel'] : $_SESSION['squhuo_tel'];
				$order['receiveaddres'] = $_SESSION['receiveaddres'];
				$order['sendTime'] = $_SESSION['sendTime'];
			   // $order['quhuo_ma'] = rand(1000, 9999);
				$order['quhuo_fangshi'] =$_SESSION['shouhuo_type'] == 1 ? $_SESSION['gquhuo_fangshi'] : $_SESSION['squhuo_fangshi'];
			}
		}
        ...
        do
        {
			...
			{
			 if(isset($order['quhuo_tel'])){
					if($order['quhuo_tel']=='')
					{
						$dianhua=$order['mobile'];
					}
					else
					{
						$dianhua=$order['quhuo_tel'];
					}
				}
				$tihuoma=rand(100000,999999);
				$order['order_status'] = OS_CONFIRMED;
				$order['confirm_time'] = gmtime();
				$order['pay_status']   = PS_PAYED;
				$order['pay_time']     = gmtime();
				$order['order_amount'] = 0;
				$order['quhuo_ma']      =$tihuoma;
				$neirong=array($order['order_sn'],$tihuoma);
				$sendid='5374';		
				include_once('includes/Send.php');
				sendTemplateSMS($dianhua,$neirong,$sendid);
			}
			//print_r($order);exit;
	...
	$sqll = " SELECT goods_id ,goods_number".
        ...
        //获取商家邮箱
        $admin_userid = $admin_id?$admin_id:1;
        $sql = "select email from ".$ecs->table('admin_user')." where user_id = ".$admin_userid;
        $admin_email = $db->getOne($sql);
        ...
        if ($admin_email != '')
        {
            ...
            send_mail($_CFG['shop_name'], $admin_email, $tpl['template_subject'], $content, $tpl['is_html']);
        }
        ...
        $order['surplus'] = $order_left_surplus;
        //unset($order['order_id']);
        $order_all_amount += $order['order_amount'];
    }
    
	unset($_SESSION['daigou_id'], $_SESSION['daishou_id'], $_SESSION['shouhuo_type'], $_SESSION['gquhuo_name'], $_SESSION['squhuo_name'], $_SESSION['gquhuo_tel'], $_SESSION['squhuo_tel']);
    ...
    /* 取得支付信息，生成支付代码 */
    if ($order_all_amount > 0)
    {
        $order['order_amount'] = $order_all_amount;
        $payment = payment_info($order['pay_id']);
    ...
    $order['order_amount_formated'] = price_format($order_all_amount,false);
    /* 订单信息 */
    $smarty->assign('order',      $order);
    ...
    unset($_SESSION['shouhuo_type']);
    unset($_SESSION['daishou_id']);
    unset($_SESSION['daigou_id']);
 
    unset($_SESSION['gquhuo_name']);
    unset($_SESSION['gquhuo_tel']);
    unset($_SESSION['gquhuo_fangshi']);
    unset($_SESSION['squhuo_name']);
    unset($_SESSION['squhuo_tel']);
	    unset($_SESSION['receiveaddres']);
    unset($_SESSION['sendTime']);
    unset($_SESSION['squhuo_fangshi']);
}
...
function flow_update_cart($arr)
{
    ...
    foreach ($arr AS $key => $val)
    {
        ...
        $sql = "SELECT g.goods_name, g.goods_number, g.max_number ".
        ...
        {
			if ($row['max_number'] > 0 && $val > $row['max_number'])
            {
