...
function get_user_default($user_id)
{
    ...
    $sql = "SELECT total_points, consume_points, today_money_date, today_money_points, today_consume_date, today_consume, today_consume_points, today_ziti_date, today_ziti, today_ziti_points, pay_points, user_money, user_rank, user_rankz,credit_line, last_login, is_validated FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '$user_id'";
    $row = $GLOBALS['db']->getRow($sql);
    $info = array();
    $info['user_id'] = $user_id;
    $info['user_rank'] = $row['user_rank'];
    $info['user_rankz'] = $row['user_rankz'];
    $info['username']  = stripslashes($_SESSION['user_name']);
    $info['today_money_date'] = $row['today_money_date'];
    $info['total_points'] = $row['total_points'];
    $info['pay_points'] = $row['pay_points'];
    $info['user_money'] = $row['user_money'];
    $info['today_money_points'] = $row['today_money_points'];
    $info['today_consume_date'] = $row['today_consume_date'];
    $info['today_consume'] = $row['today_consume'];
    $info['today_consume_points'] = $row['today_consume_points'];
    $info['today_ziti_date'] = $row['today_ziti_date'];
    $info['today_ziti'] = $row['today_ziti'];
    $info['today_ziti_points'] = $row['today_ziti_points'];
    ...
    if($info['today_ziti_date'] < $today_date){
        $info['today_ziti'] = 0;
        $info['today_ziti_points'] = 0;
    }
    if($info['today_consume_date'] < $today_date){
        $info['today_consume'] = 0;
        $info['today_consume_points'] = 0;
    }
    $info['today_ziti_points'] = $info['today_ziti_points']*1;
    $info['today_consume_points'] = $info['today_consume_points']*1;
    $info['consume_points'] = $row['consume_points'] * 1;
   
    if($info['today_money_date'] < $today_date){
        if($info['user_money'] > 0){
            $info['today_money_points'] = round($info['user_money'] * 0.0002, 4);
            if($info['today_money_points']>0){
                $info['pay_points'] += $info['today_money_points'];
                $info['total_points'] += $info['today_money_points'];
                $sql = "update ".$GLOBALS['ecs']->table('users') ." set today_money_date = {$today_date}, today_money_points = ".$info['today_money_points']." where user_id=".$info['user_id'];
                if($GLOBALS['db']->query($sql)){
                    log_account_change($info['user_id'], 0, 0, 0, $info['today_money_points'], '每日余额积分');
                }
            }
        }else{
            $info['today_money_points'] = 0;
            $sql = "update ".$GLOBALS['ecs']->table('users') ." set today_money_points = ".$info['today_money_points']." where user_id=".$info['user_id'];
            $GLOBALS['db']->query($sql);
        }
    }
    $info['total_points'] = $info['total_points']*1;
    $info['pay_points'] = $info['pay_points']*1;
    $info['today_money_points'] = $info['today_money_points']*1;
    $info['today_points'] = ($info['today_money_points'] + $info['today_ziti_points'] + $info['today_consume_points'])*1;
