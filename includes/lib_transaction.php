 ...
 */
function get_user_orders($user_id, $num = 10, $start = 0, $type = 0, $where = '')
{
    /* 取得订单列表 */
    $arr    = array();
    ...
    $sql = "SELECT order_id, order_sn, order_status, shipping_status, pay_status, add_time,user_id,quhuo_name,quhuo_tel,quhuo_ma, quhuo_fangshi, " .
           "(goods_amount + shipping_fee + insure_fee + pay_fee + pack_fee + card_fee + tax - discount) AS total_fee,dai_status ".
           " FROM " .$GLOBALS['ecs']->table('order_info') .
           " WHERE {$column} = '$user_id' {$where} ORDER BY add_time DESC";
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $row['order_status_name'] = $GLOBALS['_LANG']['os'][$row['order_status']];
        $row['shipping_status_name'] = $GLOBALS['_LANG']['ss'][$row['shipping_status']];
        $row['pay_status_name'] = $GLOBALS['_LANG']['ps'][$row['pay_status']];
        
        $arr[] = array('order_id'       => $row['order_id'],
					'quhuo_name'	=> $row['quhuo_name'],
					'quhuo_tel' => $row['quhuo_tel'],
					'quhuo_ma' => $row['quhuo_ma'],
                    'quhuo_fangshi' => $row['quhuo_fangshi'],
					   'user_id'		=> $row['user_id'],
                       ...
                       'shipping_status_name' => $row['shipping_status_name'],
                       'pay_status' => $row['pay_status'],
                       'shipping_status' => $row['shipping_status'],
                       'dai_status' => $row['dai_status'],
                       'handler'        => $row['handler']);
    }
 ...
 * @return  bool        $bool
 */
function affirm_daohuo($order_id, $user_id = 0)
{
    /* 查询订单信息，检查状态 */
    $sql = "SELECT user_id, order_sn , order_status, shipping_status, pay_status,daishou_id,daigou_id FROM ".$GLOBALS['ecs']->table('order_info') ." WHERE order_id = '$order_id'";

    $order = $GLOBALS['db']->GetRow($sql);
	//echo $user_id."---".$order['daishou_id']."----------".$order['daigou_id'];

    // 如果用户ID大于 0 。检查订单是否属于该用户
    if ($user_id > 0 && ($order['daishou_id'] != $user_id && $order['daigou_id'] != $user_id))
    {//echo "213";exit;
    ...
    {
	//echo $order['shipping_status'].SS_SHIPPED;exit;
	//echo dfd;exit;
    ...
    {
        $sql = "UPDATE " . $GLOBALS['ecs']->table('order_info') . " SET dai_status = '1' WHERE order_id = '$order_id'";
        
        if ($GLOBALS['db']->query($sql))
        {
            /* 记录日志 */
            order_action($order['order_sn'], $order['order_status'], SS_RECEIVED, $order['pay_status'], '网点到货', $GLOBALS['_LANG']['buyer']);
 ...
 * @return  bool        $bool
 */
function affirm_shouhuo($order_id, $user_id = 0,$quhuo_ma)
{
    /* 查询订单信息，检查状态 */
    $sql = "SELECT user_id, order_sn , order_status, shipping_status, pay_status,dai_status, daishou_id,daigou_id,quhuo_fangshi,quhuo_ma FROM ".$GLOBALS['ecs']->table('order_info') ." WHERE order_id = '$order_id'";
    ...
    {
	//echo "&&&&&&&&&&".$order['shipping_status'] ;
	//exit;
        $sql = "UPDATE " . $GLOBALS['ecs']->table('order_info') . " SET dai_status = '2' WHERE order_id = '$order_id'";
        if($GLOBALS['db']->query($sql)){
            $sql = "update " . $GLOBALS['ecs']->table('users') . " set user_rankz = 1 where user_id = ".$order['user_id']." ";
            $GLOBALS['db']->query($sql);
            if($order['dai_status'] !=2 && $order['quhuo_fangshi'] == 1){
                ziti_points($order);
            }
        }
    ...
    else
    {
		...
		if( $quhuo_ma == $order['quhuo_ma']){
		
					$sql = "UPDATE " . $GLOBALS['ecs']->table('order_info') . " SET dai_status = '2' WHERE order_id = '$order_id'";
					$sql2 = "UPDATE " . $GLOBALS['ecs']->table('order_info') . " SET shipping_status = '" . SS_RECEIVED . "' WHERE order_id = '$order_id'";
					
					if ($GLOBALS['db']->query($sql) && $GLOBALS['db']->query($sql2))
					{
						$sql = "update " . $GLOBALS['ecs']->table('users') . " set user_rankz = 1 where user_id = ".$order['user_id']." and user_rankz = 0";
						$GLOBALS['db']->query($sql);
						if($order['dai_status'] !=2 && $order['quhuo_fangshi'] == 1){
							ziti_points($order);
						}
						/* 记录日志 */
						order_action($order['order_sn'], $order['order_status'], SS_RECEIVED, $order['pay_status'], '客户收货', $GLOBALS['_LANG']['buyer']);
		    ...
		    echo "<script>alert('提货码不正确,请重新输入!');history.back();</script>";
		}
...
function get_shop_list(){
    $sql = "select msn from ".$GLOBALS['ecs']->table('users')." where user_rank = 3 and msn!=''";
...
/*自提赠送积分*/
function ziti_points($order){
    $user_id = intval($order['user_id']);
    $sql = "SELECT user_id,today_ziti_date,today_ziti FROM ".$GLOBALS['ecs']->table('users') ." WHERE user_id = '$user_id'";
    ...
    if($users){
        ...
        if($users['today_ziti_date']<$today_date){
            
            $sql = "update " . $GLOBALS['ecs']->table('users') . " set today_ziti_date = '$today_date', today_ziti = 1, today_ziti_points = {$point} where user_id = '$user_id'";
            
        }else{
            $sql = "update " . $GLOBALS['ecs']->table('users') . " set today_ziti_points = today_ziti_points + {$point} where user_id = '$user_id'";
            
        }
        if($GLOBALS['db']->query($sql)){
            log_account_change($user_id, 0, 0, 0, $point, '自提赠送积分');
        }
...
function save_consignee($consignee, $default=false)
{
    ...
    //{
        ...
        $sql = "UPDATE " . $GLOBALS['ecs']->table('users') .
            " SET address_id = '$consignee[address_id]', country='$consignee[country]', province='$consignee[province]', city='$consignee[city]', district='$consignee[district]' WHERE user_id = '$_SESSION[user_id]'";
...
function update_address($address)
{
    ...
    //{
        $sql = "UPDATE ".$GLOBALS['ecs']->table('users') .
                " SET address_id = '".$address_id."',country='".$address['country']."',province='".$address['province']."',city='".$address['city']."',district='".$address['district']."' ".
