...
function order_fee($order, $goods, $consignee)
{
    ...
    if ($order['shipping_id'] > 0 && $total['real_goods_count'] > 0)
    {
        ...
        if (!empty($shipping_info))
        {
            if ($order['extension_code'] == 'group_buy')
            {
                $weight_price = cart_weight_price($goodsids,CART_GROUP_BUY_GOODS);
            }
            else
            {
                $weight_price = cart_weight_price($goodsids);
            }
 ...
 * @return  array
 */
function order_allfee($order, $moregoods, $consignee)
{
    /* 初始化订单的扩展code */
    if (!isset($order['extension_code']))
    {
    ...
    foreach($moregoods as $admin_id => $goods){
        $goods_price = $market_price = $real_goods_count = 0;
        /* 商品总价 */
        $goodsids = array();
        foreach ($goods AS $val)
        {
        ...
        if ($order['shipping_id'] > 0 && $real_goods_count > 0)
        {
            $region['country']  = $consignee['country'];
            $region['province'] = $consignee['province'];
            $region['city']     = $consignee['city'];
            ...
            if (!empty($shipping_info))
            {
                ...
                $shipping_fee= ($shipping_count == 0 AND $weight_price['free_shipping'] == 1) ?0 :  shipping_fee($shipping_info['shipping_code'],$shipping_info['configure'], $weight_price['weight'], $goods_price, $weight_price['number']);
                ...
                $total['shipfees'][$admin_id] = price_format($shipping_fee, false);
                $total['shipping_fee'] += $shipping_fee;
            }
        }
        
        $total['goods_price'] += $goods_price;
        $total['market_price'] += $market_price;
    }
    $total['saving']    = $total['market_price'] - $total['goods_price'];
    $total['save_rate'] = $total['market_price'] ? round($total['saving'] * 100 / $total['market_price']) . '%' : 0;
 ...
 */
function cart_admin_goods($type = CART_GENERAL_GOODS)
{
    ...
    $admin_goods = array();
    foreach($arr as $key=>$value){
        $admin_id = $GLOBALS['db']->getOne("select admin_id from ". $GLOBALS['ecs']->table('goods') ." where goods_id =".$value['goods_id']);
        $admins = $GLOBALS['db']->getRow("select role_id,name,province,city,district from ". $GLOBALS['ecs']->table('admin_user') ." where user_id = ".$admin_id);
        if($admins && $admins['role_id']){
            $value['adminname'] = $admins['name'];
            $value['areaname'] = '';
            if($admins['province']){
                $value['areaname'] .= $GLOBALS['db']->getOne('select region_name from '.$GLOBALS['ecs']->table('region').' where region_id='.$admins['province']);
            }
            if($admins['city']){
                $value['areaname'] .= $GLOBALS['db']->getOne('select region_name from '.$GLOBALS['ecs']->table('region').' where region_id='.$admins['city']);
            }
            if($admins['district']){
                $value['areaname'] .= $GLOBALS['db']->getOne('select region_name from '.$GLOBALS['ecs']->table('region').' where region_id='.$admins['district']);
            }
            
            $admin_goods[$admin_id][] = $value;
        }else{
            $value['adminname'] = '手拉手商城';
            $value['areaname'] = '';
            $admin_goods[0][] = $value;
        }
    }
 ...
 * @return  array
 */
function cart_weight_price($goodsids, $type = CART_GENERAL_GOODS)
{
    ...
    /* 计算超值礼包内商品的相关配送参数 */
    $sql = 'SELECT goods_id, goods_number, goods_price FROM ' . $GLOBALS['ecs']->table('cart') . " WHERE extension_code = 'package_buy' AND session_id = '" . SESS_ID . "' and goods_id in (".implode(',', $goodsids).")";
    ...
    $sql    = 'SELECT SUM(g.goods_weight * c.goods_number) AS weight, ' .
                ...
                "WHERE c.session_id = '" . SESS_ID . "'  and c.goods_id in (".implode(',', $goodsids).") " .
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
{
    ...
    /* 取得商品信息 */
    $sql = "SELECT g.goods_name, g.goods_sn, g.is_on_sale, g.is_real, g.admin_id, ".
...
function integral_to_give($order)
{
    ...
    {
        $order_money = $order['surplus']+$order['order_amount'];
        ...
        $sql = "SELECT SUM(og.goods_number * og.goods_price * 0.05) AS custom_points, SUM(og.goods_number * IF(g.rank_integral > -1, g.rank_integral, og.goods_price)) AS rank_points " .
        ...
        if($rows){
            $rows['custom_points'] = round($order_money * (0.05),4);
        }
        return $rows;
    }
}
